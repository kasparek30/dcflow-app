...
type SupplierMaterialImportResult = unknown;

function clean(value: unknown) {
  return String(value || "").replace(/\s+/g, " ").trim();
}

function materialOrderNum(value: unknown): number | null {
  const n = Number(value);
  return Number.isFinite(n) ? n : null;
}

function safeMaterialOrderIdPart(value: unknown) {
  return clean(value)
    .replace(/[^\w.-]+/g, "_")
    .replace(/^_+|_+$/g, "")
    .slice(0, 80);
...
function getMaterialOrderLineDescription(line: Record<string, unknown>) {
  return (
...
function getMaterialOrderLineQuantity(line: Record<string, unknown>) {
  return (
    materialOrderNum(line.quantity) ??
    materialOrderNum(line.qty) ??
    materialOrderNum(line.orderedQty) ??
    materialOrderNum(line.shippedQty) ??
    1
  );
}

function getMaterialOrderLineUnitCost(line: Record<string, unknown>) {
  return (
    materialOrderNum(line.unitCost) ??
    materialOrderNum(line.unitPrice) ??
    materialOrderNum(line.price) ??
    materialOrderNum(line.rate) ??
    null
  );
}

function getMaterialOrderLineTotal(
  line: Record<string, unknown>,
  quantity: number,
  unitCost: number | null
) {
  return (
    materialOrderNum(line.lineTotal) ??
    materialOrderNum(line.total) ??
    materialOrderNum(line.amount) ??
    (unitCost !== null ? Math.round(quantity * unitCost * 100) / 100 : null)
  );
}

function buildMaterialOrderImportedLineItems(args: {
  poCode: string;
  ...
  return lines
    .map((line: any, index: number) => {
      const description = getMaterialOrderLineDescription(line);
      const quantity = getMaterialOrderLineQuantity(line);
      const unitCost = getMaterialOrderLineUnitCost(line);
      const totalCost = getMaterialOrderLineTotal(line, quantity, unitCost);

      const stableId =
        safeMaterialOrderIdPart(
          [
            "supplier",
            args.poCode,
...
function upsertMaterialOrderSupplierInvoice(args: {
  existing: unknown;
  poCode: string;
  supplierInvoiceId: string;
  parsedInvoice: ReturnType<typeof parseSupplierInvoiceText> | null;
  ...
  const parsed = args.parsedInvoice as any;
  ...
  const incomingInvoiceNumber = clean(parsed?.invoiceNumber);
...
function replaceMaterialOrderImportedLineItemsForInvoice(args: {
  existing: unknown;
  poCode: string;
  supplierInvoiceId: string;
...
function updateMaterialOrderPurchaseOrderLinks(args: {
  existing: unknown;
  poCode: string;
  supplierInvoiceId: string;
  parsedInvoice: ReturnType<typeof parseSupplierInvoiceText> | null;
...
function computeMaterialOrderSupplierCostTotal(args: {
  ...
  const invoiceTotal = invoices.reduce((sum, invoice: any) => {
    const value = materialOrderNum(invoice?.total);
    return sum + (value ?? 0);
  }, 0);
  ...
  const lineTotal = lines.reduce((sum, line: any) => {
    const value = materialOrderNum(line?.totalCost);
    return sum + (value ?? 0);
  }, 0);
...
function getNextMaterialOrderStatus(currentStatus: unknown) {
...
async function importSupplierMaterialsToMaterialOrder(args: {
  poCode: string;
  supplierInvoiceId: string;
  parsedInvoice: ReturnType<typeof parseSupplierInvoiceText> | null;
}) {
  ...
  const importedLines = buildMaterialOrderImportedLineItems({
    poCode,
    supplierInvoiceId,
    parsedInvoice: args.parsedInvoice,
  });

  const nextSupplierInvoices = upsertMaterialOrderSupplierInvoice({
  ...
  const nextLineItems = replaceMaterialOrderImportedLineItemsForInvoice({
  ...
  const nextPurchaseOrders = updateMaterialOrderPurchaseOrderLinks({
  ...
  const supplierCostTotal = computeMaterialOrderSupplierCostTotal({
    supplierInvoices: nextSupplierInvoices,
    lineItems: nextLineItems,
  });

  const nextStatus = getNextMaterialOrderStatus(materialOrder.status);
...
async function appendAttachmentToPurchaseOrder(args: {
    ...
    extractedMeta: {
      ...
      extractionMethod:
        String(args.attachment.extractedMeta?.supplierParser || "") === "moore_batch_page"
          ...
          : String(args.attachment.extractedMeta?.supplierParser || "") === "supplyhouse_email_body"
            ? "email_body_rendered_or_generated_pdf"
            : "native_unpdf",
  ...
  const attachmentWasRefreshed = existingAttachmentIndex >= 0;
